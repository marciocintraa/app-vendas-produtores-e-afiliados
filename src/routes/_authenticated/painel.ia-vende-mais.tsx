...
import { ArrowLeft, Check, Copy, Loader2, Lock, Wand2 } from "lucide-react";
import { analyzeProduct, type VendeMaisResult } from "@/lib/vende-mais.functions";
...
export const Route = createFileRoute("/_authenticated/painel/ia-vende-mais")({
  component: VendeMaisPage,
  head: () => ({
    meta: [
      { title: "IA VENDE+ — Vende Fácil Pro" },
      { name: "description", content: "Monte a estrutura comercial do seu produto: público, benefícios, objeções e chamadas." },
      { property: "og:title", content: "IA VENDE+ — Vende Fácil Pro" },
      { property: "og:description", content: "Monte a estrutura comercial do seu produto em poucos minutos." },
...
function resultToText(result: VendeMaisResult) {
  return result.sections
    .map((s, i) => `${i + 1}. ${s.title}\n${s.items.map((it) => `- ${it}`).join("\n")}`)
    .join("\n\n");
}

function VendeMaisPage() {
    ...
    <div className="min-h-screen bg-[#070B18] px-5 py-8 text-white">
      <div className="mx-auto max-w-5xl">
        ...
        <header className="mt-5 flex items-center gap-3">
          <span className="rounded-xl bg-cyan-500/10 p-2.5 text-cyan-300"><Wand2 className="h-5 w-5" /></span>
          <div>
            <h1 className="text-2xl font-bold">IA VENDE+</h1>
            <p className="text-sm text-slate-400">
              Informe os dados do produto e receba a estrutura comercial pronta: público, problema, benefícios,
              argumentos, objeções e chamadas.
            </p>
          </div>
        </header>
        ...
        {!checking && !hasAccess && (
          <section className="mt-8 rounded-2xl border border-cyan-400/20 bg-[#0A0F22] p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-400">
              A IA VENDE+ monta toda a estrutura de venda do seu produto. Ela faz parte do Plano PRO, junto com
            ...
            <section className="mt-7 rounded-2xl border border-white/5 bg-[#0A0F22] p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input className={input} value={form.category} onChange={(e) => set("category")(e.target.value)} placeholder="Ex.: Marketing" />
                </div>
                <div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Cole aqui trechos da página de vendas, promessas, bônus, garantia…"
              ...
              >
                {mutation.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisando…</> : <><Wand2 className="h-4 w-4" /> Gerar análise</>}
              </button>

              {mutation.isError && (
                <p className="mt-4 rounded-xl border border-red-500/20 bg-red-500/5 p-4 text-sm text-red-300">
                  {(mutation.error as Error)?.message || "Não foi possível gerar a análise agora."}
                </p>
              )}
            </section>
