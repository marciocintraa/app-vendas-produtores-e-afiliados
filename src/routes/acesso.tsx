...
const buildAccessLink = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<AccessResult> => {
    ...
    if (error || !link.properties?.action_link) {
      ...
      logDelivery({ step: 'access', email, success: false, detail: error?.message ?? 'empty action_link' });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-6 py-12">
      <div className="max-w-lg w-full rounded-2xl border border-border/50 bg-card p-8 md:p-10 text-center shadow-2xl">
        ...
        {(state === 'no_purchase' || state === 'inactive' || state === 'link_failed' || state === 'invalid_email') && (
          <>
            {state === 'no_purchase' && polling && (
              <>
                ...
                <p className="text-muted-foreground mt-2">
                  Estamos confirmando o pagamento. Assim que o acesso for liberado, você será redirecionado
                  automaticamente.
                </p>
            ...
            {state === 'no_purchase' && !polling && (
              <>
                ...
                <p className="text-muted-foreground mt-2">
                  Ainda não recebemos a confirmação da sua compra para <strong>{email}</strong>. Se acabou de comprar,
                  aguarde alguns minutos e tente novamente. Se pagou via boleto, a liberação ocorre após a compensação
                  (1 a 2 dias úteis).
                </p>
                <button
                  onClick={handleRetry}
            ...
            {state === 'invalid_email' && (
              <>
                ...
                <p className="text-muted-foreground mt-2">
                  O endereço de email fornecido não parece válido. Verifique o link enviado pela Hotmart.
                </p>
              </>
            )}
          </>
