...
import { Loader2, Search, CheckCircle, AlertCircle, Mail, ArrowLeft, PackageOpen } from 'lucide-react';
        ...
        <div className="mt-6">
          ...
          {status === 'found' && (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Encontramos sua assinatura {planLabel(result?.plan)} para <strong>{result?.email}</strong>.
              </p>
              <a
          ...
          {status === 'not_found' && (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Não localizamos uma assinatura ativa para <strong>{result?.email}</strong>. Se acabou de comprar,
          ...
          {status === 'inactive' && (
            <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Sua assinatura {planLabel(result?.plan)} para <strong>{result?.email}</strong> está cancelada ou
                expirada. Reative na Hotmart para voltar a acessar.
              </p>
            </div>
          )}
          ...
          {status === 'error' && (
            <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Não conseguimos verificar agora. Tente novamente em instantes.</p>
            </div>
          )}
        </div>
      </div>
