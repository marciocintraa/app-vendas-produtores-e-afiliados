...
async function postWebhook(payload) {
  const res = await fetch(
    `${BASE_URL}/api/public/hotmart/webhook?hottok=${encodeURIComponent(HOTTOK)}`,
    {
      method: "POST",
...
(async () => {
  ...
  for (const offer of OFFERS) {
    ...
    const redirected =
      ac.status >= 300 &&
      ac.status < 400 &&
      /supabase\.co\/auth\/v1\/verify|access_token=/.test(ac.location);
    step(
      redirected,
      `[${offer.label}] /acesso redireciona para magic link`,
      redirected
        ? `→ ${ac.location.slice(0, 80)}...`
        : `status=${ac.status} location="${ac.location}"`,
    );
  ...
  step(
    rr.status === 200 && rr.body?.ok === true,
    `PURCHASE_REFUNDED processado sem erro`,
    `status=${rr.status}`,
  );
  ...
  step(
    ...
    reactivatedRedirect
      ? `→ ${afterReactivate.location.slice(0, 80)}...`
